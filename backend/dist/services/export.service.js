...
exports.createExportArchive = createExportArchive;
const archiver_1 = __importDefault(require("archiver"));
const stream_1 = require("stream");
const codegen_service_1 = require("./codegen.service");
function createExportArchive(suite) {
    ...
    for (const test of suite.tests) {
        const code = (0, codegen_service_1.generateTestCode)(test, suite);
    ...
    // full suite file
    archive.append((0, codegen_service_1.generateSuiteCode)(suite), { name: 'tests/full-suite.spec.ts' });
    // README
...
// ─── File builders ─────────────────────────────────────────────────────────────
function buildPackageJson(suite) {
    return {
...
function buildPlaywrightConfig(suite) {
...
function buildGHActionsWorkflow(suite) {
...
function buildPageObjectFile(po, suite) {
