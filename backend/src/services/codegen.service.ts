 ...
 */
function injectHoverForNavMenus(steps: Step[], nextId: () => string): Step[] {
  const isNavLinkClick = (st: Step): st is ClickStep =>
    st.type === 'click' &&
    (st as ClickStep).locatorType === 'role' &&
    (st as ClickStep).selector === 'link';

  const isNavLinkHover = (st: Step): st is HoverStep =>
    st.type === 'hover' &&
    (st as HoverStep).locatorType === 'role' &&
    (st as HoverStep).selector === 'link';

  const makeWait = (): WaitStep => ({
    id: nextId(),
    type: 'wait',
