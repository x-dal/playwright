import type {
  Step, TestSuite, Test, PageObject, TestDataSet,
  NavigateStep, ClickStep, TypeStep, SelectStep, WaitStep,
  AssertStep, PageObjectStep, ConditionalStep, HoverStep,
  KeyboardStep, ScrollStep, ScreenshotStep,
} from '../db/models';
// ClickStep, HoverStep, WaitStep kept for renderClick/renderHover/renderWait renderers above
import type { LocatorType, LocatorOptions } from '../db/models';
import { buildLocatorCode, esc } from './locator';

// ─── Main entry point ──────────────────────────────────────────────────────────

export function generateSuiteCode(suite: TestSuite): string {
  const lines: string[] = [];
  ...
  lines.push(`test.describe('${esc(suite.name)}', () => {`);
...
export function generateTestCode(test: Test, suite: TestSuite): string {
  const lines: string[] = [];
...
function generatePageObjects(pageObjects: PageObject[]): string {
  return pageObjects
    .map(po => {
...
function generatePageObjectHelpers(po: PageObject): string {
  return '';
}

// ─── Test generation ───────────────────────────────────────────────────────────

function generateTest(test: Test, suite: TestSuite): string {
  const lines: string[] = [];
  ...
  if (dataSet && dataSet.columns.length > 0) {
    ...
    lines.push(`    test(\`${esc(test.name)} - \${JSON.stringify(data)}\`, async ({ page }) => {`);
    lines.push(renderSteps(test.steps, dataSet.columns.map(c => c.key), 6));
    lines.push(`    });`);
    lines.push(`  }`);
  } else {
    lines.push(`  test('${esc(test.name)}', async ({ page }) => {`);
...
function renderSteps(steps: Step[], dataKeys: string[], indent: number): string {
...
function renderStep(step: Step, dataKeys: string[]): string {
  switch (step.type) {
    ...
    default:          return `// unknown step type: ${(step as any).type}`;
  }
}

function renderNavigate(s: NavigateStep): string {
  return `await page.goto('${esc(s.url)}');`;
}

function renderClick(s: ClickStep): string {
  const opts: Record<string, any> = {};
  if (s.button && s.button !== 'left') opts.button = s.button;
  if (s.clickCount && s.clickCount > 1) opts.clickCount = s.clickCount;
  const optsStr = Object.keys(opts).length > 0 ? `, ${JSON.stringify(opts)}` : '';
  return `await ${buildLocatorCode(s.locatorType, s.selector, s.locatorOptions)}.click(${optsStr.slice(2) || ''});`;
}

function renderType(s: TypeStep, dataKeys: string[]): string {
  const value = interpolateDataKeys(s.value, dataKeys);
  const fill = s.clearFirst === false
    ? `await ${buildLocatorCode(s.locatorType, s.selector, s.locatorOptions)}.pressSequentially(${value});`
    : `await ${buildLocatorCode(s.locatorType, s.selector, s.locatorOptions)}.fill(${value});`;
  return fill;
}

function renderSelect(s: SelectStep, dataKeys: string[]): string {
  const value = interpolateDataKeys(s.value, dataKeys);
  return `await ${buildLocatorCode(s.locatorType, s.selector, s.locatorOptions)}.selectOption(${value});`;
}

function renderWait(s: WaitStep): string {
  switch (s.waitType) {
    case 'timeout':
    ...
    case 'selector':
    default: {
      const core = `await page.locator('${esc(s.selector ?? '')}').waitFor({ timeout: ${s.timeout} });`;
      if (s.onTimeout === 'skip') {
...
function renderAssert(s: AssertStep): string {
  const loc = buildLocatorCode(s.locatorType, s.selector, s.locatorOptions);
  switch (s.assertType) {
    case 'containsText':    return `await expect(${loc}).toContainText('${esc(s.expected)}');`;
    case 'notContainsText': return `await expect(${loc}).not.toContainText('${esc(s.expected)}');`;
    case 'isVisible':       return `await expect(${loc}).toBeVisible();`;
    case 'isHidden':        return `await expect(${loc}).toBeHidden();`;
    case 'hasValue':        return `await expect(${loc}).toHaveValue('${esc(s.expected)}');`;
    case 'hasCount':        return `await expect(${loc}).toHaveCount(${Number(s.expected)});`;
    case 'urlContains':     return `await expect(page).toHaveURL(/${esc(s.expected)}/);`;
    case 'titleContains':   return `await expect(page).toHaveTitle(/${esc(s.expected)}/);`;
...
function renderPageObjectCall(s: PageObjectStep): string {
  const paramValues = Object.values(s.params).map(v => `'${esc(v)}'`).join(', ');
  const sep = paramValues ? ', ' : '';
  return `await ${toCamelCase(s.pageObjectId)}(page${sep}${paramValues});`;
}

function renderConditional(s: ConditionalStep, dataKeys: string[]): string {
  const cond = s.condition;
  let check: string;
  switch (cond.type) {
    case 'elementVisible':
      check = `await ${buildLocatorCode(cond.locatorType, cond.selector ?? '', cond.locatorOptions)}.isVisible()`;
      break;
    case 'elementContainsText':
      check = `(await ${buildLocatorCode(cond.locatorType, cond.selector ?? '', cond.locatorOptions)}.textContent() ?? '').includes('${esc(cond.value ?? '')}')`;
      break;
    case 'urlContains':
      check = `page.url().includes('${esc(cond.value ?? '')}')`;
      break;
    default:
      check = 'true';
  }
...
function renderHover(s: HoverStep): string {
  return `await ${buildLocatorCode(s.locatorType, s.selector, s.locatorOptions)}.hover();`;
}

function renderKeyboard(s: KeyboardStep): string {
  const press = `await page.keyboard.press('${esc(s.key)}');`;
...
function renderScroll(s: ScrollStep): string {
  if (s.selector) {
    return `await ${buildLocatorCode(s.locatorType, s.selector, s.locatorOptions)}.scrollIntoViewIfNeeded();`;
  }
  return `await page.evaluate(() => window.scrollTo(${s.x ?? 0}, ${s.y ?? 0}));`;
}

function renderScreenshot(s: ScreenshotStep): string {
  const name = s.screenshotName ? `'${esc(s.screenshotName)}.png'` : `\`screenshot-\${Date.now()}.png\``;
...
function interpolateDataKeys(value: string, dataKeys: string[]): string {
  if (dataKeys.length === 0) return `'${esc(value)}'`;
  let result = value;
  let hasInterp = false;
  ...
  return hasInterp ? `\`${result}\`` : `'${esc(value)}'`;
}

// ─── Parse recorded Playwright codegen output into Steps ──────────────────────

const METHOD_TO_LOCATOR: Record<string, LocatorType> = {
...
export function parseCodegenOutput(code: string): Step[] {
  ...
  for (const raw of code.split('\n')) {
    ...
    if (/await page\.waitForTimeout\(/.test(line)) continue;
    ...
    if (line.startsWith('await ')) {
      ...
      if (parsed && parsed.locatorExpr.startsWith('page.')) {
        ...
        const raw_patch = { selector: locatorExpr, locatorType: 'raw' as const };
 ...
 */
function splitLocatorAction(line: string): { locatorExpr: string; action: string; argsRaw: string } | null {
  ...
  for (let i = 0; i < stripped.length; i++) {
    const ch = stripped[i];
    if (inStr) {
      if (ch === inStr && stripped[i - 1] !== '\\') inStr = null;
      continue;
    }
  ...
  if (stripped[argsOpenIdx] !== '(') return null;
 ...
 */
function deduplicateRawSteps(steps: Step[]): Step[] {
  const result: Step[] = [];
  for (let i = 0; i < steps.length; i++) {
    ...
    if (
      'locatorType' in curr && (curr as any).locatorType === 'raw' &&
      next && next.type === curr.type &&
      'locatorType' in next && (next as any).locatorType !== 'raw'
    ) {
      continue;
    }
    result.push(curr);
...
function describeLocator(t: LocatorType, sel: string, opts?: LocatorOptions): string {
  if (t === 'role') return opts?.name ? `[${sel}] "${opts.name}"` : `[${sel}]`;
  if (t === 'css')  return sel;
  return `${t}:"${sel}"`;
}
 ...
 */
function scanBalancedArgs(line: string, openIdx: number): { raw: string; closeIdx: number } | null {
  let depth = 0;
  let inStr: string | null = null;
  for (let i = openIdx; i < line.length; i++) {
    ...
    if (ch === '"' || ch === "'" || ch === '`') { inStr = ch; continue; }
    if (ch === '(') { depth++; continue; }
    if (ch === ')') { if (--depth === 0) return { raw: line.slice(openIdx + 1, i), closeIdx: i }; }
  }
  return null;
}
 ...
 */
function parseLocatorArgs(raw: string): { selector: string; options?: LocatorOptions } {
  const trimmed = raw.trim();
  // Single/double/backtick string only → simple CSS/role value
  const simpleMatch = trimmed.match(/^(['"`])(.*?)\1$/s);
  if (simpleMatch) return { selector: simpleMatch[2] };

  // "firstArg, { ...options }" form
  const quoteChar = trimmed[0];
  if (quoteChar !== "'" && quoteChar !== '"' && quoteChar !== '`') return { selector: trimmed };
  const firstQuoteEnd = trimmed.indexOf(quoteChar, 1);
  if (firstQuoteEnd === -1) return { selector: trimmed };
  ...
  while ((m = kvRe.exec(optionsPart)) !== null) {
    const key = m[1] as keyof LocatorOptions;
    if (m[3] !== undefined) (options as any)[key] = m[3];
    else if (m[4] === 'true') (options as any)[key] = true;
    else if (m[4] === 'false') (options as any)[key] = false;
    else if (m[4]) (options as any)[key] = Number(m[4]);
  }
