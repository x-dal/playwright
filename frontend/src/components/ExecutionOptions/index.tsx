  ...
  return (
    <div className="h-full overflow-y-auto p-6 max-w-2xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-lg font-semibold text-slate-900 dark:text-white">Execution Options</h3>
        {dirty && (
      ...
      <div className="space-y-6">
        ...
        <section className="card p-5">
          <h4 className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-3">Target Browsers</h4>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-600 mt-2">Tests run once per selected browser. {opts.browsers.length} browser(s) selected.</p>
        </section>

        {/* Headless & slow-mo */}
        <section className="card p-5 space-y-4">
          <h4 className="text-sm font-semibold text-slate-700 dark:text-slate-300">Browser Behavior</h4>
          <div className="flex items-center justify-between">
            <div>
              <div className="text-sm font-medium text-slate-800 dark:text-slate-200">Headless Mode</div>
              <div className="text-xs text-slate-500 dark:text-slate-500">Run without visible browser window (faster, good for CI)</div>
            </div>
            <button
              onClick={() => patch({ headless: !opts.headless })}
              className={`relative w-12 h-6 rounded-full transition-colors ${opts.headless ? 'bg-blue-600' : 'bg-slate-300 dark:bg-slate-600'}`}
            >
              <span className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-transform ${opts.headless ? 'left-7' : 'left-1'}`} />
            </button>
          </div>
          <div>
            <label className="label">Slow Mo (ms) <span className="text-slate-400 dark:text-slate-600">– delay between actions</span></label>
            <div className="flex items-center gap-3">
              ...
              />
              <span className="text-sm text-slate-700 dark:text-slate-300 w-16 text-right tabular-nums">{opts.slowMo ?? 0}ms</span>
            </div>
          </div>
        </section>
        ...
        {/* Viewport & Device */}
        <section className="card p-5 space-y-4">
          <h4 className="text-sm font-semibold text-slate-700 dark:text-slate-300">Viewport &amp; Device</h4>
          <div>
          ...
          <div>
            <label className="label">Device Emulation <span className="text-slate-400 dark:text-slate-600">(overrides viewport)</span></label>
        ...
        {/* Timeouts & Retries */}
        <section className="card p-5 space-y-4">
          <h4 className="text-sm font-semibold text-slate-700 dark:text-slate-300">Timeouts &amp; Retries</h4>
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-xs text-slate-400 dark:text-slate-600 mt-1">Per-action timeout for clicks, fills, asserts</p>
            </div>
            <div>
              ...
              <p className="text-xs text-slate-400 dark:text-slate-600 mt-1">Retry a failed test up to N times</p>
            </div>
          </div>
          <div>
            <label className="label">Base URL <span className="text-slate-400 dark:text-slate-600">(optional prefix for relative navigate steps)</span></label>
